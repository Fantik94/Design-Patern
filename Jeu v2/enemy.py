...
class Enemy:
    ...
    def __init__(self, name, base_health_points, base_attack_damage, rarity_weights=None, floor_number=1):
        ...
        self.base_attack_damage = base_attack_damage * (1.07 ** (floor_number - 1)) * (1.09 ** (floor_number - 1)) * self.RARITY_DAMAGE_MULTIPLIERS[self.rarity]
    ...
    def display_health(self):
        ...
        return f"{self.color}♥ {self.health_points:.1f} [{health_bar}]{Style.RESET_ALL}"

    def display_info(self):
        return f"{self.color}{self.name} ({self.type}, {self.rarity}) - Points de vie : {self.health_points:.1f}{Style.RESET_ALL}"
