...
def main():
    ...
    for floor_number in range(1, 11):
        ...
        while ennemi.health_points > 0 and personnage.health_points > 0:
            ...
            if damage == float('inf'):
                ...
                print(f"{Fore.YELLOW}Dégâts infligés : {damage:.1f}{Fore.RESET}")
                if critical:
                    print(f"{Fore.RED}Coup critique !")
                ennemi.take_damage(damage)

                lifesteal_amount = personnage.lifesteal(damage, lifesteal)
                print(f"{Fore.GREEN}Vous récupérez {lifesteal_amount:.1f} points de vie grâce au vol de vie !")

                game_manager.add_battle_history(
                    f"{Fore.CYAN}Étage {floor_number}: {personnage.attack_strategy.attack()} - Dégâts : {damage:.1f} {'(Coup critique!)' if critical else ''} (Vol de vie : {lifesteal_amount:.1f})\n"
                    f"{Fore.BLUE}Joueur ({personnage.name}) - Points de vie : {personnage.health_points:.1f}{Style.RESET_ALL}\n"
                    f"{Fore.RED}Ennemi ({ennemi.name}) - Points de vie : {ennemi.health_points:.1f}{Style.RESET_ALL}"
                )
            ...
            if ennemi.health_points > 0:
                ...
                print(f"\n{Fore.RED}{ennemi.name} attaque et inflige {damage:.1f} dégâts!")
                print(f"{personnage.name} - Barre de vie : {personnage.display_health()}")
                game_manager.add_battle_history(
                    f"{Fore.RED}Étage {floor_number}: {ennemi.name} attaque - Dégâts : {damage:.1f}\n"
                    f"{Fore.BLUE}Joueur ({personnage.name}) - Points de vie : {personnage.health_points:.1f}{Style.RESET_ALL}\n"
                    f"{Fore.RED}Ennemi ({ennemi.name}) - Points de vie : {ennemi.health_points:.1f}{Style.RESET_ALL}"
                )
        ...
        if personnage.health_points > 0:
            ...
            print(f"\n{Fore.GREEN}{personnage.name} se soigne de {heal_amount:.1f} points de vie à la fin de l'étage!")
            print(f"{personnage.name} - Barre de vie : {personnage.display_health()}")
    ...
    if personnage.health_points > 0:
        ...
        print(f"\n{Fore.RED}{'='*20} FIN DU JEU {'='*20}\nVoici l'histoire des combats :\n{'='*20} FIN DU JEU {'='*20}{Style.RESET_ALL}")
